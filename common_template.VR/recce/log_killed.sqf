...
log_death = {
	...
	if (_unit getVariable ["is_special", false]) then {
		SPECIAL_TOTAL_KILLED = SPECIAL_TOTAL_KILLED + 1
	};
	diag_log "**** Killed event detected ****";
	diag_log format ["%1 (side %2) was killed. Total deaths so far: East: %3, West: %4, Independent: %5, Civilian: %6, Special: %7",
		name _unit, 
		_side, 
		EAST_TOTAL_KILLED, 
		WEST_TOTAL_KILLED, 
		INDEPENDENT_TOTAL_KILLED, 
		CIVILIAN_TOTAL_KILLED,
		SPECIAL_TOTAL_KILLED
	];
};
